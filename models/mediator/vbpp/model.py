...
def integrate_log_fn_sqr(mean, var):
    ...
    point_eval = tf.math.log(mean ** 2 + var)  # TODO use mvnquad instead?
    # TODO explain
...
class VBPP(gpflow.models.GPModel, gpflow.models.ExternalDataTrainingLossMixin):
    ...
    def elbo(self, events):
        ...
        """
        K = gpflow.covariances.Kuu(self.inducing_variable, self.kernel, jitter=self.Kuu_jitter)

        integral_term = self._elbo_integral_term(K)
        data_term = to_default_float(0.0)
        for evs in events:
            if evs.shape[0] > 0:
                data_term += self._elbo_data_term(evs, K)
    ...
    def predict_log_density_lbo(self, new_events):
        ...
        O = len(new_events)
        elbo = O * integral_term + data_term
        return elbo

    def predict_integral_term(self):
        K = gpflow.covariances.Kuu(self.inducing_variable, self.kernel)
        integral_term = self._elbo_integral_term(K)
